...
| `o.SigningCredentials = new SigningCredentials(symmetricKey, "HS256")` | v1 pins RS256. Use `RsaSecurityKey` with `RS256`. Validator rejects symmetric keys, HMAC algorithms, ECDsa keys, and every non-RS256 algorithm at startup. |
...
| `o.SigningCredentials = new SigningCredentials(new JsonWebKey(...), "RS256")` | `JsonWebKey` rejected at startup (JWKS converter throws on JsonWebKey input). Use `RsaSecurityKey`. |
