...
public class TodosControllerTests
{
    ...
    [Fact]
    public async Task Create_under_v2_response_includes_IsOverdue_field()
    {
        // v2 (2026-12-01) added `isOverdue` to TodoResponse. This test asserts the
        // field is present (and false for a future-dated todo) when the request hits
        // api-version=2026-12-01 — paired with the v1 absence assertion that proves
        // namespace versioning keeps the two response shapes isolated.
        var client = CreateClient("user-1", "todos:create");
        ...
        response.StatusCode.Should().Be(HttpStatusCode.Created);
        var todo = await response.Content.ReadAsAsyncWithAssertion<TodoResponse>();
        todo.IsOverdue.Should().BeFalse("a future-dated todo is not overdue");

        var json = await response.Content.ReadAsStringAsync(TestContext.Current.CancellationToken);
        json.Should().Contain("isOverdue", "v2 response shape exposes the IsOverdue field");
    }
